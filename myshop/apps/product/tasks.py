...
from .bl import parse_xlsx_file
...
def process_import_task(file_import_id):
    try:
        ...
        file_import.approved = len([product_data for product_data in product_data_list if is_approved(product_data)])
        file_import.rejected = len([product_data for product_data in product_data_list if is_rejected(product_data)])
